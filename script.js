...
class RestaurantManager {
    ...
    loadInitialData() {
        ...
        if (Object.values(this.menu).every(category => category.length === 0)) {
            this.menu = {
                entrantes: [
                    { id: 1, name: 'Bruschetta Mediterránea', price: 8.50, description: 'Pan tostado con tomate, albahaca y mozzarella fresca' },
                    { id: 2, name: 'Carpaccio de Res', price: 12.90, description: 'Finas láminas de res con rúcula, parmesano y vinagreta' }
                ],
                principales: [
                    { id: 3, name: 'Paella Valenciana', price: 18.50, description: 'Arroz con pollo, conejo, verduras y azafrán auténtico' },
                    { id: 4, name: 'Salmón a la Plancha', price: 22.00, description: 'Salmón fresco con verduras asadas y salsa de eneldo' },
                    { id: 5, name: 'Risotto de Setas', price: 16.50, description: 'Arroz cremoso con setas variadas y queso parmesano' }
                ],
                postres: [
                    { id: 6, name: 'Tiramisú Casero', price: 6.50, description: 'Postre italiano tradicional con café y mascarpone' },
                    { id: 7, name: 'Tarta de Chocolate', price: 7.20, description: 'Intenso chocolate belga con helado de vainilla' }
                ],
                bebidas: [
                    { id: 8, name: 'Vino Tinto Reserva', price: 4.50, description: 'Copa de vino tinto de la casa' },
                    { id: 9, name: 'Agua Mineral', price: 2.50, description: 'Agua mineral natural con o sin gas' },
                    { id: 10, name: 'Café Espresso', price: 2.80, description: 'Café espresso italiano auténtico' }
                ]
            };
            this.saveMenu();
        }
    ...
    loadSocialLinks() {
        // Actualizar enlaces de redes sociales con valores por defecto
        ...
        if (instagramLink) {
            instagramLink.href = this.socialLinks.instagram || 'https://instagram.com/labuenamesa';
        }
        if (facebookLink) {
            facebookLink.href = this.socialLinks.facebook || 'https://facebook.com/labuenamesa';
        }
        if (whatsappLink) {
            whatsappLink.href = this.socialLinks.whatsapp || `https://wa.me/${this.contactInfo.phone.replace(/\D/g, '')}`;
        }
        if (emailLink) {
    ...
    switchSection(sectionName) {
        // Remover clase active de todos los nav items y secciones
        ...
        // Activar el nav item y sección correspondiente
    ...
    // Método para limpiar y reinicializar datos
    resetToDefaults() {
        ...
        this.loadInitialData();
        this.init();
        this.showSuccessMessage('Datos reinicializados a valores por defecto');
    }
}
...
function resetToDefaults() {
    if (confirm('¿Estás seguro de que quieres reinicializar todos los datos a valores por defecto? Esta acción no se puede deshacer.')) {
        restaurantManager.resetToDefaults();
    }
}
        ...
        <div class="admin-content" style="max-width: 500px;">
            ...
            <div style="padding: 1rem;">
                ${items.length === 0 ?
                    ...
                    items.map(item => `
                        <div class="menu-item" style="margin-bottom: 1rem; padding: 1rem; background: var(--surface-light); border: 1px solid var(--border); border-radius: var(--border-radius);">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('Menú cargado:', restaurantManager.menu);
    console.log('Enlaces cargados:', restaurantManager.mainLinks);

    // Forzar renderizado después de un pequeño delay
    setTimeout(() => {
        restaurantManager.renderMenu();
        restaurantManager.renderMainLinks();
    }, 100);
});
